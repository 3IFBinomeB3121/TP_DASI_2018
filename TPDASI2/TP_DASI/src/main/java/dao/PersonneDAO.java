...
public class PersonneDAO {
    ...
    public static List<Personne> verifierExistencePersonne(String mail, String mdp){
        ...
        Query query2 = em.createQuery("select e from Employe e where "
                + "e.mail=:mailAverif AND e.motdepasse=:mdpAverifier");
        query.setParameter("mailAverif", mail);
        query.setParameter("mdpAverifier", mdp);
        query2.setParameter("mailAverif", mail);
        query2.setParameter("mdpAverifier", mdp);
        
        List<Personne> listPers = query.getResultList();
        listPers.addAll(query2.getResultList());
        return listPers;
    }
