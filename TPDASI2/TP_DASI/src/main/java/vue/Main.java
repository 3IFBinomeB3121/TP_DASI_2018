    ...
    public static void main(String[] args) {
        ...
        switch (scenario){
            ...
            case 2:
                ...
                List<Intervention> interEnCours = Service.consulterInterventionEnCours(emp);
                if (interEnCours != null){
                    System.out.println(interEnCours.toString());
                }
                else{
                ...
                List<Intervention> listInterv = Service.consulterOpeDuJour();
                for (Intervention intervention : listInterv){
            ...
            default: // Par défaut, on lance nos tests unitaires
                ...
                // Test de consulterInterventionEnCours
                
                Client clientInterEnCours = new Client("Monsieur", "Locare", "Valentin", dateFictive, "1900 route de Vins, 83143, LE VAL", "employe@hotmail.fr", "0630276677", "coucou");
                Client clientInterEnCoursInscrit = Service.inscrireClient(clientInterEnCours);
                System.out.println(clientInterEnCoursInscrit.toString());
                Saisie.pause();
                System.out.println("Le client demande l'intervention\r\n");
                Incident inter = new Incident("Test de consulterInterventionEnCours");
                Intervention interTestConsult = Service.demanderIntervention(clientsInfos.get(0), inter);
                System.out.println(interTestConsult.toString() + "\r\n");
                Saisie.pause();
                List<Intervention> interEmpEnCours = Service.consulterInterventionEnCours(interTestConsult.getEmploye());
                if (interEmpEnCours != null){
                    System.out.println(interEmpEnCours.toString());
                }
                else{
                    System.out.println("Aucune intervention en cours");
                }
                Saisie.pause();
                System.out.println("On confirme la fin de l'intervention et on réessaye d'afficher"
                        + " l'intervention en cours");
                Service.confirmerFinIntervention(interTestConsult, "Terminée", "Aucun commentaire particulier");
                List<Intervention> interEmpTerm = Service.consulterInterventionEnCours(interTestConsult.getEmploye());
                if (interEmpTerm != null){
                    System.out.println(interEmpTerm.toString());
                }
                else{
                    System.out.println("Aucune intervention en cours");
                }
                ...
                Incident inter1 = new Incident("Fuite d'eau dans le sous-sol");
                ...
                Intervention intervention1 = Service.demanderIntervention(cli, inter1);
                ...
                System.out.println("Operations du jour :\r\n ");
                List<Intervention> interToday = Service.consulterOpeDuJour();
                numeroInter = 1;
                for (Intervention intervention : interToday){
                    System.out.println("Intervention numéro " + numeroInter + " :\r\n");
                    System.out.println(intervention.toString() + "\r\n");
                    numeroInter++;
                }
